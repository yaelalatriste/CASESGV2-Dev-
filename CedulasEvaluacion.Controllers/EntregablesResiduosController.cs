...
namespace CedulasEvaluacion.Controllers
{
    public class EntregablesResiduosController : Controller
    {
        ...
        public EntregablesResiduosController(IRepositorioEntregablesResiduos evResiduos, IHostingEnvironment environment)
        {
        ...
        {
            int success = 0;
            success = await eResiduos.entregableFactura(entregables);
            if (success != 0)
            {
                return Ok();
            }
        ...
        [HttpGet]
        [Route("/residuos/getEntregables/{id?}")]
        public async Task<IActionResult> getEntregablesResiduos(int id)
        {
            List<Entregables> entregables = null;
            entregables = await eResiduos.getEntregables(id);
            string table = "";
            string tipo = "";
            if (entregables != null)
            {
        ...
        [HttpGet]
        [Route("/residuos/getListadoEntregables/{id?}")]
        public async Task<IActionResult> getListadoEntregablesResiduos(int id)
        {
            List<Entregables> entregables = null;
            entregables = await eResiduos.getEntregables(id);
            if (entregables != null)
        ...
        {
            int success = 0;
            success = await eResiduos.eliminaArchivo(entregable);
            if (success != -1)
            {
                return Ok();
            }
