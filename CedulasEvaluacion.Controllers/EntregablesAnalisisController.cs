...
namespace CedulasEvaluacion.Controllers
{
    public class EntregablesAnalisisController : Controller
    {
        ...
        [HttpPost]
        [Route("/analisis/adjuntaEntregable")]
        ...
        [HttpGet]
        [Route("/analisis/getEntregables/{id?}")]
        public async Task<IActionResult> getEntregablesAnalisis(int id)
        {
            List<Entregables> entregables = null;
            entregables = await eAnalisis.getEntregables(id);
            string table = "";
            string tipo = "";
            if (entregables != null)
            {
        ...
        [HttpGet]
        [Route("/analisis/getListadoEntregables/{id?}")]
        public async Task<IActionResult> getListadoEntregablesAnalisis(int id)
        {
            List<Entregables> entregables = null;
            entregables = await eAnalisis.getEntregables(id);
            if (entregables != null)
        ...
        [HttpGet]
        [Route("/analisis/verArchivo/{folio?}/{nombre?}")]
        ...
        [HttpGet]
        [Route("/analisis/buscaEntregable/{id?}/{tipo?}")]
        public async Task<IActionResult> buscaEntregable(int id, string tipo)
        {
            int exists = 0;
            exists = await eAnalisis.buscaEntregable(id, tipo);
            if (exists != -1)
            {
                return Ok(exists);
            }
        ...
        [HttpPost]
        [Route("/analisis/Entregables/autoRecha")]
        public async Task<IActionResult> aprovacionRechazoCedula([FromBody] Entregables entregables)
        {
            int success = await eAnalisis.apruebaRechazaEntregable(entregables);
            if (success != 0)
            {
                return Ok();
            }
        ...
        [HttpPost]
        [Route("/analisis/entregables/historialEntregable")]
        public async Task<IActionResult> historialEntregable([FromBody] HistorialEntregables historialEntregables)
        {
            int success = 0;
            success = await eAnalisis.capturaHistorial(historialEntregables);
            if (success != 0)
            {
                return Ok();
            }
