...
{
    public class RepositorioEntregablesAgua: IRepositorioEntregablesAgua
    {
        private readonly string _connectionString;

        public RepositorioEntregablesAgua(IConfiguration configuration)
        {
        ...
        public async Task<List<Entregables>> getEntregables(int cedula)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_getEntregablesAguaByCedula", sql))
                    {
        ...
        public async Task<int> entregableFactura(Entregables entregables)
        {
            ...
            try
            {
                if (saveFile.Equals("Ok"))
                {
                    using (SqlConnection sql = new SqlConnection(_connectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_insertarActualizarEntregableAgua", sql))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            ...
                            cmd.Parameters.Add(new SqlParameter("@cedulaAguaId", entregables.CedulaAguaId));
        ...
        public async Task<int> eliminaArchivo(Entregables entregable)
        {
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_eliminaEntregableAgua", sql))
                    {
