...
{
    public class RepositorioEntregablesMuebles : IRepositorioEntregablesMuebles
    {
        private readonly string _connectionString;

        public RepositorioEntregablesMuebles(IConfiguration configuration)
        {
        ...
        public async Task<List<Entregables>> getEntregables(int cedula)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_getEntregablesMueblesByCedula", sql))
                    {
        ...
        public async Task<int> entregableFactura(Entregables entregables)
        {
            ...
            try
            {
                if (saveFile.Equals("Ok"))
                {
                    using (SqlConnection sql = new SqlConnection(_connectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_insertarActualizarEntregableMuebles", sql))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            ...
                            cmd.Parameters.Add(new SqlParameter("@cedulaMuebleId", entregables.CedulaMuebleId));
        ...
        public async Task<int> eliminaArchivo(Entregables entregable)
        {
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_eliminaEntregableMuebles", sql))
                    {
            ...
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_autorizarRechazarEntregable", sql))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@cedulaId", entregables.CedulaMuebleId));
