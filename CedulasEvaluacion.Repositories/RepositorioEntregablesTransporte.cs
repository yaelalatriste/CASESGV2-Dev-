...
{
    public class RepositorioEntregablesTransporte : IRepositorioEntregablesTransporte
    {
        private readonly string _connectionString;

        public RepositorioEntregablesTransporte(IConfiguration configuration)
        {
        ...
        public async Task<List<Entregables>> getEntregables(int cedula)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_getEntregablesTransporteByCedula", sql))
                    {
        ...
        public async Task<int> entregableFactura(Entregables entregables)
        {
            ...
            try
            {
                if (saveFile.Equals("Ok"))
                {
                    using (SqlConnection sql = new SqlConnection(_connectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_insertarActualizarEntregableTransporte", sql))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            ...
                            cmd.Parameters.Add(new SqlParameter("@cedulaTransporteId", entregables.CedulaTransporteId));
        ...
        public async Task<int> eliminaArchivo(Entregables entregable)
        {
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_eliminaEntregableTransporte", sql))
                    {
