...
{
    public class RepositorioEntregablesCelular : IRepositorioEntregablesCelular
    {
        /************************************************ Servicio 5 - Telefonia Celular**********************************/
        private readonly string _connectionString;

        public RepositorioEntregablesCelular(IConfiguration configuration)
        {
        ...
        public async Task<List<Entregables>> getEntregables(int cedula)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_getEntregablesCelularByCedula", sql))
                    {
        ...
        public async Task<int> entregableFactura(Entregables entregables)
        {
            ...
            try
            {
                if (saveFile.Equals("Ok"))
                {
                    using (SqlConnection sql = new SqlConnection(_connectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_insertarActualizarEntregableCelular", sql))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            ...
                            cmd.Parameters.Add(new SqlParameter("@cedulaCelularId", entregables.CedulaCelularId));
        ...
        public async Task<int> eliminaArchivo(Entregables entregable)
        {
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_eliminaEntregableCelular", sql))
                    {
            ...
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_autorizarRechazarEntregable", sql))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@cedulaId", entregables.CedulaCelularId));
        ...
        private Entregables MapToValue(SqlDataReader reader)
        {
            return new Entregables
            {
                ...
                FechaPresentacion = Convert.ToDateTime(reader["FechaPresentacion"].ToString()),
