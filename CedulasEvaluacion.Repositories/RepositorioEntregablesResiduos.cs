...
{
    public class RepositorioEntregablesResiduos : IRepositorioEntregablesResiduos
    {
        /************************************************ Servicio 5 - Telefonia Celular**********************************/
        private readonly string _connectionString;

        public RepositorioEntregablesResiduos(IConfiguration configuration)
        {
        ...
        public async Task<List<Entregables>> getEntregables(int cedula)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_getEntregablesResiduosByCedula", sql))
                    {
        ...
        public async Task<int> entregableFactura(Entregables entregables)
        {
            ...
            try
            {
                if (saveFile.Equals("Ok"))
                {
                    using (SqlConnection sql = new SqlConnection(_connectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_insertarActualizarEntregableResiduos", sql))
                        {
                            cmd.CommandType = CommandType.StoredProcedure;
                            ...
                            cmd.Parameters.Add(new SqlParameter("@cedulaResiduosId", entregables.CedulaResiduosId));
        ...
        public async Task<int> eliminaArchivo(Entregables entregable)
        {
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_eliminaEntregableResiduos", sql))
                    {
            ...
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_autorizarRechazarEntregable", sql))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@cedulaId", entregables.CedulaResiduosId));
        ...
        private Entregables MapToValue(SqlDataReader reader)
        {
            return new Entregables
            {
                ...
                Estatus = reader["Estatus"].ToString().Trim(),
