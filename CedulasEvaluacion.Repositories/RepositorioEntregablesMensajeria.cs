...
{
    public class RepositorioEntregablesMensajeria : IRepositorioEntregablesMensajeria
    {

        /************************************************* Servicio 3 - Mensajeria Acelerada *****************************************************/
        private readonly string _connectionString;

        public RepositorioEntregablesMensajeria(IConfiguration configuration)
        {
        ...
        public async Task<List<Entregables>> getEntregables(int cedula)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("sp_getEntregablesMensajeriaByCedula", sql))
                    {
        ...
        public async Task<int> entregableFactura(Entregables entregables)
        {
            ...
            try
            {
                if (saveFile.Equals("Ok"))
                {
                    using (SqlConnection sql = new SqlConnection(_connectionString))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_insertarActualizarEntregableMensajeria", sql))
                        {
                            ...
                            cmd.Parameters.Add(new SqlParameter("@cedulaMensajeriaId", entregables.CedulaMensajeriaId));
        ...
        public async Task<int> eliminaArchivo(Entregables entregable)
        {
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_eliminaEntregableMensajeria", sql))
                    {
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
            ...
            try
            {
                using (SqlConnection sql = new SqlConnection(_connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("sp_autorizarRechazarEntregable", sql))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@servicio", "Mensajería Acelerada"));
